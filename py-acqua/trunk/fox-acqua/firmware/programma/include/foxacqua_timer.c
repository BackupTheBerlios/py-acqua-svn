...
#include "foxacqua_timer.h"



int read_timer(s_timer *ret, int num) {
	...
	memcpy(&(ret->stato),buffer[2*sizeof(char)+sizeof(int)],sizeof(char));
	
}

int read_timers(s_timer **ret) {
...
int set_timer(s_timer *ret) {
	...
	memcpy(buffer[2*sizeof(char)+sizeof(int)],&(ret->stato),sizeof(char));
...
int set_timers(s_timer **ret, int max) {
	//salva i timer fino al max tra 0 e N_SONDE*N_TIMER

	perror("non implementata");
	
}

int del_timer(int num) {
...
static int timer_file(int status) {
	...
	switch (status){
		case 1:
			...
			if(!fseek(timer_fd, N_TIMER*N_SONDE*RECORD-1, SEEK_SET))
