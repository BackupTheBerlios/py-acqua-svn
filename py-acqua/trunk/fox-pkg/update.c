...
int update (void) {
    ...
    if (n < 0)
        ...
        while(n--) {
			local_ver[ret].name = strdup(strtok(namelist[n]->d_name,"_"));
	...
	for (ptr =0; ptr <= n; ptr++) {
		...
		for (inx =0; inx < app_count; inx ++) {
			...
			if (!strcmp(local_ver[ptr].name, remote_ver[inx].name)) {
				//ok i nomi sono uguali verifichiamo la versione				
				if (strcmp(local_ver[ptr].ver, remote_ver[inx].ver)>0) {
					upgrade (&local_ver[ptr], &remote_ver[inx]);
				}
			}
		}
	}
