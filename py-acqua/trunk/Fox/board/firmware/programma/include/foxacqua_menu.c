...
time_t now;
	...
	strftime ( conv, sizeof conv, "%H", tm_now );	hr=atoi(conv);
	strftime ( conv, sizeof conv, "%M", tm_now );	min=atoi(conv);
	strftime ( conv, sizeof conv, "%d", tm_now );	gg=atoi(conv);
	strftime ( conv, sizeof conv, "%m", tm_now );	mese=atoi(conv); 
	strftime ( conv, sizeof conv, "%y", tm_now );	anno=atoi(conv);
	...
	strcpy(buff,"date ");
	if (mese<10 ) strcat(buff,"0");
	strcat(buff,itoa(mese));
	if (gg<10 ) strcat(buff,"0");
	strcat(buff,itoa(gg));
	if (hr<10 ) strcat(buff,"0");
	strcat(buff,itoa(hr));
	if (min<10 ) strcat(buff,"0");
