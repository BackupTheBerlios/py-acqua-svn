...
void sk_menu_1_2_3_1(){//IMPOSTA TIMER
int presa,stato,timer,i,hr,min,sec,year,mth;
   ...
   for(i=0;i<10;i++) if (presa_timer_stato[presa][i]==99){ //CONTROLLA SE � LIBERO
       timer=i;//trovato!
         goto SKIP;
   }
SKIP:
   ...
   //   hr = rm_bcd(read_ds1302(0x85));//recupera ora attuale
...
//RIMOSTRA:
      ...
      for(i=0;i<3;i++){
         ...
         if( presa_timer_stato[presa][num]<99 ){
            lcd_printf("%d:%d => %d ",presa_timer[presa][num][0],presa_timer[presa][num][1],presa_timer_stato[presa][num]);
             }
         else lcd_printf("--:-- => -");
         num++;
      }
   //disegna barra
      if (j==0) barra_menu_vert(0); //j==limite inf
      else if (j==2) barra_menu_vert(2); //j==limite sup
      else barra_menu_vert(1);//casi centrali
   ...
   //	if (scelta==0) => mostra i sucessivi
      if (scelta>=1){
         if (scelta<4){
            ...
            if (j==0) mod_timer(presa,scelta-1); // MODIFICA QUESTO TIMER
                                                 // scelta va da 1 a 3,
                                                 // il timer dv andare da 0 a 2
            else if (j==1) mod_timer(presa,scelta+2);
            else if (j==2) mod_timer(presa,scelta+5);
...
void sk_menu_1_2_3(){
   ...
   lcd_printf("Elimina timer");
	...
	if (scelta==0) ;//ritorna
	else if (scelta==1) sk_menu_1_2_3_1(); // imposta timer
	else if (scelta==2) sk_menu_1_2_3_2(); // mostra timer
	else if (scelta==3) sk_menu_1_2_3_3(); // elimina timer
...
unsigned char scelta;
   ...
   lcd_printf("Timer");
	...
	sk_clear();
	if (scelta==0) ;//ritorna
	else if (scelta==1) sk_menu_1_2_1();//ASSEGNA NOMI
	else if (scelta==2) sk_menu_1_2_2();//caMBIA STATO
	else if (scelta==3) sk_menu_1_2_3();//timer

}
//#inline
void sk_menu_1_1(){
   unsigned char min,hr,day,mth,year;
...
//ora
  ...
  // mth = rm_bcd ( read_ds1302 ( 0x89 ) );
   ...
   lcd_printf("%d:%d  %d/%d/%d",hr,min,day,mth,year);

	// x,y,partenza,min,max
	hr=inc_cifra(2,2,hr,0,24);// hh
	min=inc_cifra(5,2,min,0,59);// mm
	day=inc_cifra(9,2,day,0,31);// gg
	mth=inc_cifra(12,2,mth,0,12);// m
	year=inc_cifra(15,2,year,7,20);// aa
	// verifica data x gg bisestili e gg de mesi

	//se ok salva il valore
  // rtc_set_datetime(day, mth, year, hr, min);
   sk_clear();
	y_pos(4,3);
...
unsigned char sk_menu_1a(){
   ...
   lcd_printf("Gestione sonde");
	...
	scelta=aggiorna_cursore_opz(0,4);
	sk_clear();
	if (scelta==0) delay_ms(1);//ritorna
