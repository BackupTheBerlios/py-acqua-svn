...
class Invertebrati(gtk.Window):
	def __init__(self): 
		...
		self.invertebrati_store = gtk.ListStore(int, str, str, str, str, str, gtk.gdk.Pixbuf)
		
		self.view = view = gtk.TreeView(self.invertebrati_store)
		...
		cursore.execute("select * from invertebrati")

		# Costruisci l'immagine..
		for y in cursore.fetchall():
			self.invertebrati_store.append([y[0], y[1], y[2], y[3], y[4],
	...
	def on_refresh(self, widget):
		...
		if it != None:
			id = int(self.invertebrati_store.get_value(it, 0))
			...
			cur.execute("update invertebrati set date='%(date)s', vasca='%(vasca)s', quantita='%(quantita)s', nome='%(nome)s', img='%(img)s' where id = %(id)s" %vars())
			conn.commit()
			
			self.invertebrati_store.set_value(it, 1, date)
			self.invertebrati_store.set_value(it, 2, vasca)
			self.invertebrati_store.set_value(it, 3, quantita)
			self.invertebrati_store.set_value(it, 4, nome)
			self.invertebrati_store.set_value(it, 5, self.make_image(img))
			self.invertebrati_store.set_value(it, 6, img)
	...
	def on_add(self, widget):
		...
		while it != None:
			tmp = int(self.invertebrati_store.get_value(it, 0))
		...
		id += 1		
		it = self.invertebrati_store.append()

		# Settiamo il campo ID
		self.invertebrati_store.set_value(it, 0, id)
		...
		self.invertebrati_store.set_value(it, 3, quantita)
		self.invertebrati_store.set_value(it, 4, nome)
		self.invertebrati_store.set_value(it, 5, img)
		self.invertebrati_store.set_value(it, 6, self.make_image(img))
		
		conn = sqlite.connect(os.path.join('Data', 'db'))
		cur = conn.cursor()

		cur.execute('insert into invertebrati values(?,?,?,?,?,?)',
	...
	def on_del(self, widget): 
		...
		if it != None:
			...
			value = int(self.invertebrati_store.get_value(it, 0))

			# Rimuoviamo dal database
			conn = sqlite.connect(os.path.join('Data', 'db'))
			cur = conn.cursor()

			cur.execute('delete from invertebrati where id=%d' % value)
			conn.commit()

			# Rimuoviamo la riga selezionata
			self.invertebrati_store.remove(it)
			...
			while it != None:
				...
				if value < tmp:
					self.invertebrati_store.set_value(it, 0, tmp-1)
				it = mod.iter_next(it)
