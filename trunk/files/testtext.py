...
class Buffer(gtk.TextBuffer):
    ...
    def __init__(self):
        ...
        #self.invisible_tag = self.create_tag(None, invisible=gtk.TRUE)
        self.not_editable_tag = self.create_tag(editable=gtk.FALSE,
                                                foreground="purple")
        self.found_text_tag = self.create_tag(foreground="red")

        tabs = pango.TabArray(4, gtk.TRUE)
    ...
    def search_forward(self, str, view):
        self.search(str, view, gtk.TRUE)

    def search_backward(self, str, view):
        self.search(str, view, gtk.FALSE)
    ...
    def set_colors(self, enabled):
        ...
        for tag in self.color_tags:
            if enabled:
                ...
                tag.set_property("foreground_set", gtk.FALSE)
    ...
    def fill_example_buffer(self):
        ...
        for i in range(100):
            ...
            temp_mark = self.create_mark("tmp_mark", iter, gtk.TRUE);
    ...
    def save_buffer(self):
        ...
        start, end = self.get_bounds()
        chars = self.get_slice(start, end, gtk.FALSE)
        try:
...
class View(gtk.Window):
    def __init__(self, buffer=None):
        menu_items = [
            ...
            ( "/Settings/Editable", None, self.do_editable_changed, gtk.TRUE, "<RadioItem>" ),
            ( "/Settings/Not editable", None, self.do_editable_changed, gtk.FALSE, "/Settings/Editable" ),
            ( "/Settings/sep1", None, None, 0, "<Separator>" ),
            ( "/Settings/Cursor visible", None, self.do_cursor_visible_changed, gtk.TRUE, "<RadioItem>" ),
            ( "/Settings/Cursor not visible", None, self.do_cursor_visible_changed, gtk.FALSE, "/Settings/Cursor visible" ),
            ( "/Settings/sep1", None, None, 0, "<Separator>" ),
            ...
            ( "/Settings/sep1", None, None, 0, "<Separator>" ),
            ( "/Settings/Sane spacing", None, self.do_spacing_changed, gtk.FALSE, "<RadioItem>" ),
            ( "/Settings/Funky spacing", None, self.do_spacing_changed, gtk.TRUE, "/Settings/Sane spacing" ),
            ( "/Settings/sep1", None, None, 0, "<Separator>" ),
            ( "/Settings/Don't cycle color tags", None, self.do_color_cycle_changed, gtk.FALSE, "<RadioItem>" ),
            ( "/Settings/Cycle colors", None, self.do_color_cycle_changed, gtk.TRUE, "/Settings/Don't cycle color tags" ),
            ( "/_Attributes", None, None, 0, "<Branch>" ),
            ( "/Attributes/Editable", None, self.do_apply_editable, gtk.TRUE, None ),
            ( "/Attributes/Not editable", None, self.do_apply_editable, gtk.FALSE, None ),
            ( "/Attributes/Invisible", None, self.do_apply_invisible, gtk.FALSE, None ),
            ( "/Attributes/Visible", None, self.do_apply_invisible, gtk.TRUE, None ),
            ( "/Attributes/Custom tabs", None, self.do_apply_tabs, gtk.FALSE, None ),
            ( "/Attributes/Default tabs", None, self.do_apply_tabs, gtk.TRUE, None ),
            ( "/Attributes/Color cycles", None, self.do_apply_colors, gtk.TRUE, None ),
            ( "/Attributes/No colors", None, self.do_apply_colors, gtk.FALSE, None ),
    ...
    def do_insert_and_scroll(self, callback_action, widget):
        ...
        mark = buffer.create_mark(None, end, gtk.FALSE)
        ...
        self.text_view.scroll_to_mark(mark, 0, gtk.TRUE, 0.0, 1.0)
    ...
    def do_search(self, callback_action, widget):
        ...
        dialog.vbox.pack_end(search_text, gtk.TRUE, gtk.TRUE, 0)
    ...
    def cursor_set_callback(self, buffer, location, mark):
        ...
        if mark == buffer.get_insert():
            ...
            tab_window.invalidate_rect(None, gtk.FALSE)
            #tab_window.invalidate_rect(tab_window.get_geometry()[:4], gtk.FALSE)
